...
/**
 * KashPoint Logo Component
 * A modern logo featuring a stylized "K" with currency accent
 */

export function Logo({ size = 40, showText = true, className = "" }) {
  const id = useId().replace(/:/g, "");
  const gLogo = `logoGradient-${id}`;
  const gPoint = `pointGradient-${id}`;
  const gCoin = `coinGradient-${id}`;
    ...
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      >
        <defs>
          ...
          <linearGradient id={gPoint} x1="0%" y1="0%" x2="100%" y2="0%">
            <stop offset="0%" stopColor="#fbbf24" />
            <stop offset="100%" stopColor="#f59e0b" />
          </linearGradient>
          <linearGradient id={gCoin} x1="0%" y1="0%" x2="100%" y2="100%">
...
export function LogoIcon({ size = 32, className = "" }) {
  const id = useId().replace(/:/g, "");
  const gLogo = `logoGradientIcon-${id}`;
  const gCoin = `coinGradientIcon-${id}`;
...
export function LogoMinimal({ size = 24, className = "" }) {
  const id = useId().replace(/:/g, "");
  const gLogo = `logoGradientMin-${id}`;
