...
export default function Transactions() {
  ...
  // Full-screen table view; Esc or the header button leaves it.
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        <div className="flex items-center gap-0.5 p-0.5 bg-slate-100 dark:bg-slate-800/80 rounded-lg">
          ...
          <button onClick={() => setFullScreen(true)} title="Full screen (table)"
            className="px-2 py-1 rounded-md text-[11px] font-medium text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-white dark:hover:bg-slate-900 transition inline-flex items-center gap-1">
      ...
      {/* Table + pagination — rendered inline, or inside the full-screen
          panel when expanded. Same elements either way, so filters, sorting
          and selection keep working in both views. */}
      {fullScreen ? (
        <FullScreenPanel
          title="Transactions"
          meta={
            <span className="text-xs text-slate-500 dark:text-slate-400 tabular-nums truncate">
              {filtered.length.toLocaleString()} rows &middot; {fmtR(totals.revenue)} &middot; profit {fmtR(totals.profit)}
            </span>
          }
          onClose={() => setFullScreen(false)}
        >
          <div className="space-y-3">{tableSection}</div>
        </FullScreenPanel>
      ) : (
        tableSection
      )}
